...
static int connect_leader_broker(char *topic, int part_id) {
    ...
    if (!t_meta || part_id >= t_meta->partitions) return K_ERR;
    leader_id = t_meta->part_metas[part_id]->leader_id;
    if (leader_id < 0) return K_ERR;
...
int send_metadata_request(const char *topics) {
    ...
    if ((cfd = random_connect_broker()) <= 0) return K_ERR;
